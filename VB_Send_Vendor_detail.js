...
define(['N/search', 'N/https'], (search, https) => {
    ...
    const afterSubmit = (context) => {
        try {
            ...
            const vendorSearch = search.load({ id: 'customsearch1790091421405' });
            ...
            vendorSearch.run().each(result => {
                ...
                if (!vendor) {
                    vendor = {
                        ...
                        currency: result.getValue({ name: 'currency' }) || null,
            ...
            if (!vendor) {
                log.error('Vendor Search', 'No result found for Vendor ID: ' + vendorId);
                return;
            }
            ...
            search.create({
                ...
                filters: [['entity', 'anyof', vendorId]],
                columns: ['entity', 'subsidiary']
            }).run().each(result => {

                subsidiaries.push({
                    entity: String(result.getValue({ name: 'entity' })),
                    subsidiary: String(result.getValue({ name: 'subsidiary' }))
                });
            ...
            // =====================================================
            // API SEND - COMMENTED OUT FOR TESTING
